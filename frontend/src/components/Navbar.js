    ...
    return (React.createElement("nav", { className: "bg-primary text-primary-foreground border-b border-primary/20 sticky top-0 z-50" },
        React.createElement("div", { className: "container mx-auto px-4" },
            React.createElement("div", { className: "flex justify-between items-center h-16" },
                ...
                React.createElement("div", { className: "hidden md:flex space-x-4" }, filteredNavItems.map((item) => (React.createElement(Link, { key: item.name, to: item.path, className: `flex items-center px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ease-in-out
                ...
                React.createElement("div", { className: "md:hidden" },
                    React.createElement(Button, { variant: "ghost", onClick: () => setIsOpen(!isOpen), className: "text-primary-foreground" },
        ...
        React.createElement(motion.div, { className: "md:hidden", initial: "closed", animate: isOpen ? "open" : "closed", variants: {
            ...
            React.createElement("div", { className: "px-2 pt-2 pb-3 space-y-1 sm:px-3" }, filteredNavItems.map((item) => (React.createElement(Link, { key: item.name, to: item.path, className: `flex items-center px-3 py-2 rounded-md text-base font-medium ${location.pathname === item.path
                    ? 'bg-primary-foreground text-primary'
                    : 'text-primary-foreground/80 hover:bg-primary-foreground/10 hover:text-primary-foreground'}`, onClick: () => setIsOpen(false) },
                React.createElement(item.icon, { className: "w-5 h-5 mr-2" }),
                item.name)))))));
}
